...
for i = 0:rep-2
    for j = i+1:rep-1
        dist2 = dist2 + spike_dist(sps{1}(:,i*period/dt+1:(i+1)*period/dt),...
            sps{1}(:,j*period/dt+1:(j+1)*period/dt), dt, tc, kerneltype)/(((rep-1)*rep)/2);
    end
end
for i = 0:rep-2
    for j = i+1:rep-1
        dist3 = dist3 + spike_dist(sps{2}(:,i*period/dt+1:(i+1)*period/dt),...
            sps{2}(:,j*period/dt+1:(j+1)*period/dt), dt, tc, kerneltype)/(((rep-1)*rep)/2);
    end
end
